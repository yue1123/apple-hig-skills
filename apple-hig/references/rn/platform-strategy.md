...
| **tvOS** | `react-native-tvos` (fork) | Usable | Focus engine integrated; Expo works via `@react-native-tvos/config-tv` + `expo prebuild` (Expo Go does not); narrower package ecosystem |
...
**tvOS is reachable from Expo**, contrary to a common assumption — `@react-native-tvos/config-tv` swaps the runtime to the fork during `expo prebuild`. What doesn't work is Expo Go; you need a custom dev client. The tvOS app is still a separate app target with its own dependency tree, so the monorepo split above applies either way.
